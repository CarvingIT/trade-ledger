    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
			...
			<div class="text-right">
                ...
                <a class="m-5" title="New Unit" href="/admin/unit-form/new"><span class="fas fa-plus"></span></a>
                @endif
                &nbsp;
				<!--a class="m-5" title="Export" href="/admin/export/units"><span class="fas fa-file-export"></span></a-->
			</div>
			...
			<td>{{ $c->name }}</td>
			<td>{{ @$c->unit->name }}</td>
			<td>{{ $c->related_unit_quantity }}</td>
			<td>{{ \Illuminate\Support\Str::limit($c->description, 30, $end='...') }}</td>
			<td>
				<a href="/admin/unit/{{ $c->id }}" title="View Details"><span class="fas fa-eye" style="padding:5%;"></span></a>
				@if(Auth::user()->hasRole('admin'))
				<a href="/admin/unit-form/{{ $c->id }}" title="Edit"><span class="fas fa-pencil-alt" style="padding:5%;"></span></a>
				<button id="opener" class="delete_unit" data-unit-id="{{ $c->id }}" title="Delete"><span class="fas fa-trash-alt"></span></button>
				@endif

	    <div id="deletedialog" style="display:none;" class="bg-grey">
                <form name="deleteunit" method="post" action="/admin/unit/delete">
                @csrf
                <input type="hidden" id="delete_unit_id" name="unit_id" value="{{ $c->id }}" />
