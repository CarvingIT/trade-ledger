    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
	        ...
	        <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
				...
				<form name="save-purchase" action="/admin/savepurchase" method="post">
    ...
    <div class="px-4 py-5 bg-white sm:p-6 text-gray-900">
       ...
       <div class="grid grid-cols-6 gap-6">
        ...
        <div class="col-span-4 md:col-span-4">
            ...
            @endphp
             <label class="block font-medium text-sm" for="invoice_date">Purchase Date<span style="color:#F1541E;">*</span></label>
            @if(!preg_match('/new/',url()->current()))
             <input class="form-input rounded-md shadow-sm mt-1 block w-full" id="purchase_date" name="purchase_date" type="date" value="{{ $purchase->purchase_date }}" required>
            @else 
        ...
        <option value="{{ $c->id }}" @if($c->id == $purchase->entity_id) selected @endif>{{ $c->name }}</option>
        @endforeach
        </select>
        </div>
        <!-- Description -->
        <div class="col-span-8">
             <label class="block font-medium text-sm" for="description">Description (Optional)</label>
             <textarea class="form-input rounded-md shadow-sm mt-1 block w-full" id="description" name="description" type="text" placeholder="Purchased item1, item2. Still two items are remaining. Need to add them here." >{{ $purchase->description }}</textarea>
        </div>
        
        <!-- Total Amount -->
        <div class="col-span-8 md:col-span-2">
            @if(!empty($purchase->entity_id))
                @php
                    $total_amount = $purchase->total_amount;
                    $owner_entity_details = \App\Models\Entity::find($purchase->owner_entity_id);
                    $total_amount_including_tax = $purchase->total_amount;
